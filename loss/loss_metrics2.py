...
class DiceLoss(nn.Module):
    def __init__(self, weights=None, ignore_index=None, smooth=1):
    ...
    def forward(self, input, target):
        return dice_loss_per_channel(input, target, weights=self.weights, 
                                     ignore_index=self.ignore_index, smooth=self.smooth).mean()
...
#     encoded_target = Variable(encoded_target)
    ...
    numerator = 2. * numerator.sum(0).sum(1).sum(1) + smooth # sum over samples(N), then HxW
